...
class VirtualMemory:
    ...
    def accesss(self, page_id, write_mode):
        (frame_id, mapped, r, m) = self.page_table[page_id]
        if mapped:
            self.phy_mem.accesss(frame_id, write_mode)
            ...
            if len(self.freeFrames) > 0:
                ...
                self.phy_mem.accesss(new_frame_id, write_mode)
            else:
                ...
                self.phy_mem.accesss(evicted_frame_id, write_mode)
                return 1
        return 0
...
if __name__ == "__main__":
    ...
    for load in workload:
        ...
        fault_counter += vMem.accesss(page_id, acc_mode)
